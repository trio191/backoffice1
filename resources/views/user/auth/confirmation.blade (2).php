...
<body>
                            ...
                            <section class="m-auto">
                                ...
                                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                                    ...
                                    <p class="text-center mt-4" style="font-family: Poppins; font-size: 14px; font-weight: 500; line-height: 21px; letter-spacing: 1px; color: #889DC6;">
                                        Having problems? <span class="text-uppercase" style="color:#5E8BF7; font-family: Poppins;"  onclick="resendCode()">resend code</span>
                                    </p>
                                </div>
    ...
    $('.opt-input').on('input', function() {
        ...
        var nextInput = $('#' + getNextInputId(currentId));
    ...
    // Get the ID of the next input based on the current input's ID
    function getNextInputId(currentId) {
        switch (currentId) {
            case 'digit1':
                return 'digit2';
            case 'digit2':
...
function resendCode() {
    ...
    var accountSelection = JSON.parse(localStorage.getItem('accountSelection'));
    if (!accountSelection || !accountSelection.email) {
        alert("Email not found. Please restart the signup process.");
        return;
    }
    ...
    $.ajax({
        ...
        error: function(xhr) {
            alert(xhr.responseJSON?.message || "Failed to resend the code. Please try again.");
        }
    });
}

// Bind the function to the button click
$(document).on('click', '#resendCodeBtn', resendCode);
