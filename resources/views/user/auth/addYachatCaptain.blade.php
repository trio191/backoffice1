...
<body style="overflow-x:hidden">
    ...
    <script>
        $(document).ready(function() {
            // Retrieve and parse the yacht captain data from localStorage
        var addYachatCaptains = JSON.parse(localStorage.getItem('addYachatCaptain')) || {};
        ...
        $.each(fields, function(key, defaultValue) {
            var value = addYachatCaptains[key] !== undefined ? addYachatCaptains[key] : defaultValue;
            if (key === 'members') {
                // Parse the members field if it exists, otherwise use the default value
            ...
            var members = [];
            members=JSON.parse(addYachatCaptains.members)
            updateMembersList();
            ...
            $('#yachtForm').submit(function(event) {
                ...
                var isValid = validateYachtForm(yachtDetails);
                 if (isValid) {
                    localStorage.setItem('addYachatCaptain', JSON.stringify(yachtDetails));
