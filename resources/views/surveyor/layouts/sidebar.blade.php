...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme" style="background-image: url('/dashboard/assets/img/side-bg.png');background-size: 100% 70%;background-repeat: no-repeat;background-position: center bottom;">
          ...
          <ul class="menu-inner py-1">
            <!-- Dashboards -->
            <li class="menu-item {{ request()->routeIs('surveyor-dashboard.index') ? 'active' : '' }}">
              <a href="{{route('surveyor-dashboard.index')}}" class="menu-link ">
            ...
            <li class="menu-item {{ request()->routeIs('surveyor-applications.index') ? 'active' : '' }}">
              <a href="{{route('surveyor-applications.index')}}" class="menu-link ">
                <i class="menu-icon tf-icons ti ti-file-export"></i>
                <div data-i18n="Applications">Applications</div>

                @if (Application::where('user_id',Auth::id())->where('status','Under Analysis')->count()!==0)
                <div class="badge rounded-pill ms-auto" style="border: 1px solid #FFFFFF;">
                  {{Application::where('user_id',Auth::id())->where('status','Under Analysis')->count()}}
                  </div>
                  @endif
              </a>
            </li>
            <li class="menu-item {{ request()->routeIs('surveyor-reports.index') ? 'active' : '' }}">
              <a href="{{route('surveyor-reports.index')}}" class="menu-link ">
                <i class="menu-icon tf-icons ti ti-file-analytics"></i>
                <div data-i18n="Reports">Reports</div>

                @if (Report::where('reporter_id',Auth::id())->count()!==0)
                  
                
                <div class="badge  rounded-pill ms-auto" style="border: 1px solid #FFFFFF;">
                  {{Report::where('reporter_id',Auth::id())->count()}}  
                </div>
                @endif
              </a>
            </li>
            ...
            <li class="menu-item {{ request()->routeIs('surveyor-setting.index') ? 'active' : '' }}">
              <a href="{{route('surveyor-setting.index')}}" class="menu-link">
